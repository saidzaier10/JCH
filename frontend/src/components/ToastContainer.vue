    ...
    <div class="fixed top-4 right-4 z-50 flex flex-col space-y-4 pointer-events-none">
        <TransitionGroup name="toast">
            <div v-for="toast in store.toasts" :key="toast.id"
                class="pointer-events-auto flex items-center w-full max-w-xs p-4 space-x-4 text-gray-500 bg-white divide-x divide-gray-200 rounded-lg shadow dark:text-gray-400 dark:divide-gray-700 space-x dark:bg-gray-800"
                role="alert">
                ...
                <div class="shrink-0 text-sm font-normal">{{ toast.message }}</div>
                <button @click="store.remove(toast.id)" type="button"
